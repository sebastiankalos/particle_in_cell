...
def update_charge_density(particles,cells,nodes,Lx,Ly,nx,ny,h,cell_ind_matrix):
    ...
    # delete all particles that are outside the domain using list comprehension 
    # (https://stackoverflow.com/questions/1207406/how-to-remove-items-from-a-list-while-iterating):
    particles = [particle for particle in particles if (particle.locate()[0]>=0 and particle.locate()[0]<=Lx and particle.locate()[1]>=0) and particle.locate()[1]<=Ly]
    ...
    for particle in particles: #go over all particles and for each:
        ...
        if x>=0 and x<=Lx and y>=0 and y<=Ly: #if particle in the domain at all
            ...
            if i >= 0 or j >= 0: #if particle found within some cell:
                ...
                v_x = particle.read_velocity_x() + acc_x*dt/2. #calculate new velocity
                v_y = particle.read_velocity_y() + acc_y*dt/2. 

                particle.set_velocity_x(v_x) #update particle velocity
                particle.set_velocity_y(v_y)

                if update_pos:
                    x_new = particle.read_x() + v_x*dt #new position
                    y_new = particle.read_y() + v_y*dt

                    particle.set_x(x_new) #update particle position
                    particle.set_y(y_new)

                    particle_positions.append([x_new,y_new,particle.read_name(),particle.read_velocity_x(),particle.read_velocity_y()])
    if update_pos:
...
def generate_particles(particles=None,loc_x=None,loc_y=None,sigma_x=None,sigma_y=None,name=None,charge=None,mass=None,v_x=None,sigma_v_x=None,v_y=None,sigma_v_y=None,N=None):
    ...
    for i in range(N):
        particles.append(Particle(ID=int(last_ID+1), x=parts_x[i], y=parts_y[i], name=name, charge=charge, mass=mass,velocity_x=vel_x[i],velocity_y=vel_y[i],in_domain=True))
        last_ID += 1
    return